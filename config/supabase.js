...
const SUPABASE_BUCKET = process.env.SUPABASE_STORAGE_BUCKET || process.env.SUPABASE_BUCKET || 'snapvault-images';
...
function initSupabase() {
    ...
    if (!supabase) {
        supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
            ...
            global: { headers: { 'x-supabase-client': 'madiva-backend' } }
        });
...
async function listFiles(path = '', options = { limit: 1 }) {
    const client = initSupabase();
    if (!client) {
        throw new Error('Supabase storage is not configured.');
    }
